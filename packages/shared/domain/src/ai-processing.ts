...
export class AIProcessingDomain {
  private static readonly PROVIDER_COSTS: Record<AIProvider, Record<string, { input: number; output: number }>> = {
    openai: {
  ...
  static checkRateLimit(
    ...
    if (currentUsage.requests >= limits.requestsPerMinute) {
      ...
      throw new RateLimitError(
        'AI API request rate limit exceeded',
        waitTime,
        { limit: limits.requestsPerMinute, current: currentUsage.requests }
      )
    }
    ...
    if (currentUsage.tokens >= limits.tokensPerMinute) {
      const waitTime = Math.ceil((resetTime.getTime() - now.getTime()) / 1000)
      throw new RateLimitError(
        'AI API token rate limit exceeded',
        waitTime,
        { limit: limits.tokensPerMinute, current: currentUsage.tokens }
      )
    }

    if (currentUsage.cost >= limits.costPerDay) {
      throw new RateLimitError(
        'Daily AI API cost limit exceeded',
        undefined,
        { limit: limits.costPerDay, current: currentUsage.cost }
      )
    }
  }
  ...
  static aggregateUsageMetrics(
    ...
    aggregated.averageLatency = aggregated.totalRequests > 0
      ? totalLatency / aggregated.totalRequests
      : 0
    ...
    aggregated.cacheHitRate = aggregated.totalRequests > 0
      ? totalCacheHits / aggregated.totalRequests
      : 0
  ...
  static generateCacheKey(job: AIJob): string {
    const { type, payload, chatbotConfig } = job
