...
export class ConversationDomain {
  ...
  static determineResolutionStatus(messages: Message[]): 'resolved' | 'unresolved' | 'escalated' {
    const lastMessages = messages.slice(-5)

    const hasPositiveEnd = lastMessages.some(msg => msg.sentimentScore && msg.sentimentScore > 0.5)

    const hasThankYou = lastMessages.some(msg =>
      /thank you|thanks|solved|resolved|helpful/i.test(msg.content)
    )
