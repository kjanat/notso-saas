...
export class AIJobQueue {
  ...
  async addJob(job: AIJob): Promise<Bull.Job> {
    ...
    const canProcess = await this.checkTenantBudget(
      job.tenantId,
      job.metadata.costEstimate,
    );
    if (!canProcess) {
  ...
  private async processBatch(
    ...
    for (const record of batch) {
      try {
        ...
        const conversation = await this.transformRecord(
          record,
          importJob.mapping,
        );
    ...
    const summaryJob = await this.jobQueue.addJob({
      ...
      payload: {
        ...
        chatbotConfig: {
          model: "gpt-3.5-turbo",
          systemPrompt:
            "You are a conversation summarizer. Provide brief, insightful summaries.",
        },
      },
      metadata: {
        retryCount: 0,
